import { IKey, KEYS, KeyType, stringToKeys } from "./consts"
import { findLastIndex, generateOperation, multiplyOrDiviseFirst } from "./utils"

let queue:IKey[] = []

export const handleQueueInput = (input: IKey) => {
  if (queue.length > 20) return queue
  ...
  switch(input.label) {
    case '=': {
      ...
      if (queue.every(q => q.type !== KeyType.operator)) break
      if (queue[queue.length - 1].type === KeyType.operator) break
